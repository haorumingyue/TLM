...
def _advect_belt(belt_state, cells_slice, dt, boundary_in=0.0):
    """对一条皮带执行 CFL 对流并写回出流统计。返回 outflow (t)。"""
    adv, out = _advect_cells(cells_slice, belt_state.speed, 1.0, dt, boundary_in=boundary_in)
    belt_state.cells = adv
    belt_state.last_outflow_tph = out * 3600.0 / dt
    belt_state.cumulative_outflow += out
    return out


def _trim_list(lst, maxlen):
    """原地裁剪列表到最大长度，避免分配新列表。"""
    if len(lst) > maxlen:
        del lst[:-maxlen]
...
class Simulator:
    ...
    def __init__(self, fixed_speed=None):
        ...
        self.cumulative_inflow = {q: 0.0 for q in WebConfig.INFLOW_QUEUES}  # 各入流点累计入煤量 (t)
        ...
        # PID 控制器（三条皮带各一个）
        self.pids = {bid: PIDStrategy() for bid in WebConfig.BELT_ORDER}
        # 向后兼容属性
        self.pid_main = self.pids["main"]
        self.pid_incline = self.pids["incline"]
        self.pid_panel101 = self.pids["panel101"]
    ...
    def step(self) -> None:
        ...
        for qid in WebConfig.INFLOW_QUEUES:
            inflow_this_step = max(0.0, self.rates.get(qid, 0.0)) * dt
            self.queues[qid] += inflow_this_step
            self.cumulative_inflow[qid] += inflow_this_step
        ...
        # ── 2. 按皮带分别调速 ──
        prev_speeds = {}  # 记录每条皮带调速前的速度，供后续加速度计算用
        for belt_id in WebConfig.BELT_ORDER:
            state = self.belts[belt_id]
            cfg = WebConfig.BELT_CONFIGS[belt_id]
            prev_speeds[belt_id] = state.speed

            if self.fixed_speed is not None:
                state.speed = cfg.max_speed
            elif self.auto:
                pid = self.pids[belt_id]
                # 本皮带在拼接数组中的范围
                if belt_id == "main":
                    seg = cells[:n_main]
                    inflow = sum(self.rates.get(q, 0.0) for q in WebConfig.INFLOW_QUEUES)
            ...
            # 记录调速事件（速度变化时追加新事件）
            if state.speed != prev_speeds[belt_id]:
                state.speed_events[-1]["t_end"] = self.time
        ...
        main_out = _advect_belt(self.belts["main"], cells[:n_main].copy(), dt)
        inc_out = _advect_belt(self.belts["incline"], cells[n_main:n_main+n_incline].copy(), dt,
                               boundary_in=main_out)
        p101_out = _advect_belt(self.belts["panel101"], cells[n_main+n_incline:].copy(), dt,
                                boundary_in=inc_out)
        ...
        for belt_id in WebConfig.BELT_ORDER:
            ...
            accel = (state.speed - prev_speeds[belt_id]) / dt if dt > 0 else 0.0
        ...
        if int(self.total_steps) % 10 == 0:
            ...
            if len(self.t_hist) > N2:
                for lst in (self.t_hist, self.spd_hist, self.cumin_hist,
                            self.cumout_hist, self.coal_hist, self.energy_hist, self.power_hist):
                    _trim_list(lst, N2)
                for q in WebConfig.INFLOW_QUEUES:
                    _trim_list(self.flow_hist[q], N2)
    ...
    def total_in(self):
        """各入流点累计入煤量 (t)。"""
        return dict(self.cumulative_inflow)
