...
- **机械保护**: 内置"中点滞回"与"状态驻留"约束，最大限度平衡能效与机械寿命。
...
然后修改 [src/core/config.py](src/core/config.py)：
...
| `SPEED_GEARS` | `[1.5, 2.25, 3.0, 3.75, 4.5]` | 离散速度档位 (m/s) |
| `L_OPT` | `0.15` | 目标线密度 (t/m)，值越低带速越高 |
| `LOG_INTERVAL_SEC` | `3.0` | 采样间隔 (秒) |
| `PORT` | `5173` | Web 服务端口 |

更多参数调优建议详见 [速度控制策略文档](docs/SPEED_CONTROL.md#8-调优指南)。
...
| 文档 | 内容 |
| :--- | :--- |
| [速度控制策略](docs/SPEED_CONTROL.md) | PID 算法、换挡逻辑、能耗模型、调优指南 |
