...
系统支持两种时序预测后端，切换项为 [config.py](../src/core/config.py) 中的 `PREDICT_BACKEND`（`"chronos"` / `"timesfm"`）。

### 7.1 Chronos-2（默认）

- **Python 包**：`chronos-forecasting`（见 `requirements.txt`）
- **权重目录**：`models/chronos-2/`
- **实现**：[predictor.py](../src/predict/predictor.py) `_load_chronos` / `Chronos2Pipeline.predict_quantiles`

### 7.2 TimesFM（可选）

TimesFM 未列入 `requirements.txt`，需从官方仓库源码安装。安装步骤见 [requirements-timesfm.txt](../requirements-timesfm.txt)。
...
| `PREDICT_BACKEND` | config.py | `chronos` | `chronos` 或 `timesfm` |
